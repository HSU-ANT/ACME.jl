...
function model_matrices(circ::Circuit, t::Rational{BigInt})
    ...
    if sum(abs2, f_split[:c] * nullspace) > 1e-20
    ...
    return (
        ...
        fqprev_fulls = [
            reduce(
                ...
                push!(
                    fqsplit[i, 1:i-1],
                    zeros(eltype(mats[:fq]), length(model_qidxs[i]), sum(model_nns[i:end]))
                )
            )
...
if !hasmethod(cumsum, Tuple{Tuple})
    function _cumsum(x::Tuple)
        r = _cumsum(Base.front(x))
        return (r..., r[end]+x[end])
    end
    _cumsum(x::Tuple{Any}) = x
    _cumsum(::Tuple{}) = ()
end
